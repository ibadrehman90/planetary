...
def db_seed():
    ...
    earth = Planet(planet_name='Earth',planet_type='Class M',home_star='Sol',mass='5.972e24',radius='3959',distance='92.9e6')
    db.session.add(mercury)
    db.session.add(venus)
    db.session.add(earth )
    test_user = User(first_name='William', last_name='Herschel', email='abc@yahoo.com',password='1234')
...
def supersimple():
    return jsonify(message="Super simple route")
...
def login():
    # checking if the request is in JSON or form data
    if request.is_json:
...
def retrieve_password(email:str):
    ...
    if user:
        msg = Message("Your planetary account password is " + user.password,sender='donotreply@planetary.com', recipients=[email])
        mail.send(msg)
        return jsonify(message='Password sent to '+email)
    else:
        return jsonify(message='This email does not exist.'),401

# retrieving single planet's details
...
def update_planet():
    ...
    if planet:
        ...
        db.session.commit()
        return jsonify(message="Planet data updated successfully"), 202
    else:
